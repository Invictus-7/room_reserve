# Шаг_41 - задаем настройки для User
...
# асинхронный генератор get_user_db - обеспечивает доступ к БД
...
# это центральный объект библиотеки,
# связывающий созданные выше объект класса UserManager
# и бэкенд аутентификации.
# Используется для подключения роутеров
fastapi_users = FastAPIUsers[User, int](
    get_user_manager,
    [auth_backend],
)

# Вызывая соответствующие методы класса у созданного нами
# объекта fastapi_users, сохраняем в две переменных
# просто пользователя и супер-пользователя. В дальнейшим
# они нам будут нужны для DI и для работы в целом.
# Объект текущего пользователя
current_user = fastapi_users.current_user(active=True)
# Объект супер-пользователя
